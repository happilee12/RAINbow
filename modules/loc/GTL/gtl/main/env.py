...
class EnvBatch(object):
    ...
    def _init_sims(self, batch_size):
        self.sims = []
        for i in range(batch_size):
            sim = MatterSim.Simulator()
            if self.scan_data_dir:
                sim.setDatasetPath(self.scan_data_dir)
...
class GraphEnvBatch(object):
    def __init__(self, connectivity_dir, scan_data_dir=None, feat_db=None, batch_size=100, angle_feat_size=4):
        self.env = EnvBatch(connectivity_dir, scan_data_dir, feat_db, batch_size)
        ...
        if long_id not in self.buffered_state_dict:
            for ix in range(36):
                ...
                for j, loc in enumerate(state.navigableLocations[1:]):
                    ...
                    if isinstance(visual_feat, torch.Tensor) and visual_feat.is_cuda:
                        angle_feat_tensor = torch.from_numpy(angle_feat).to(visual_feat.device)
                        feat_concat = torch.cat([visual_feat, angle_feat_tensor], dim=-1)
                    else:
                        # 기존 방식으로 NumPy 배열 연결
                        if isinstance(visual_feat, torch.Tensor):
                            visual_feat_np = visual_feat.cpu().numpy()
                        else:
                            visual_feat_np = visual_feat
                        feat_concat = np.concatenate([visual_feat_np, angle_feat], axis=-1)
    ...
    def _get_obs(self):
        ...
        for i, (feature, state) in enumerate(self.env.getStates()):
            ...
            # NumPy 배열을 GPU 텐서로 변환하거나 그 반대로
            if isinstance(feature, torch.Tensor) and feature.is_cuda:
                angle_feat_tensor = torch.from_numpy(angle_feat).to(feature.device)
                feature_concat = torch.cat([feature, angle_feat_tensor], dim=-1)
            else:
                # 기존 방식으로 NumPy 배열 연결
                if isinstance(feature, torch.Tensor):
                    ...
                    feature_np = feature
                feature_concat = np.concatenate([feature_np, angle_feat], axis=-1)
    ...
    def _load_nav_graphs(self, scans):
        """
