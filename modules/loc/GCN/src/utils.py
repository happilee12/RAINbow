...
def evaluate(args, splitFile, split_name, run_name):
    # split_name = splitFile.split("_")[0]
    distance_scores = []
    splitData = json.load(open(splitFile))
    ...
    for gt in splitData:
        if "scanName" in gt:
            ...
            gt_vp = gt["start_pano"]
            pred_vp = submission[gt["instr_id"]]["viewpoint"]
            dist = geodistance_nodes[gt["scan"]][gt_vp][pred_vp]
            distance_scores.append(dist)
