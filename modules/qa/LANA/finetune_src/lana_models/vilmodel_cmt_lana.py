...
from lana_models.clip_model import Transformer
...
class T5WithImageCrossAttention(T5ForConditionalGeneration):
    def __init__(self, config):
        super().__init__(config)
        # 이미지 임베딩을 입력으로 받을 Cross-Attention Layer 추가
        self.image_attention = nn.MultiheadAttention(embed_dim=config.d_model, num_heads=config.num_heads)

    def forward(self, input_ids, attention_mask=None, decoder_input_ids=None, 
                encoder_outputs=None, labels=None, image_embedding=None):
        
        # 기존 T5 인코더 출력
        encoder_outputs = self.encoder(input_ids, attention_mask=attention_mask)
        
        # 디코더에서 이미지 임베딩을 Cross-Attention으로 활용
        if decoder_input_ids is None: 
            decoder_input_ids = torch.tensor([self.config.pad_token_id] * input_ids.size(0)).unsqueeze(1).cuda()
            # print("for inference.. decoder_input_ids", decoder_input_ids)
        ...
        query = decoder_outputs[0].transpose(0, 1) 
        key = image_embedding
        ...
        if image_embedding is not None:
            image_attention_output, _ = self.image_attention(query, key, value)
            # print("image_attention_output", image_attention_output.shape)
            # Cross-Attention 출력과 디코더 출력을 합침
            decoder_outputs = decoder_outputs[0] + image_attention_output.transpose(0, 1)
...
class LxmertEncoder(nn.Module):
    def __init__(self, config):
        ...
        if config.use_clip16:
            ...
            self.max_caption_length = 200           # NOTE match the pretraining
            self.max_caption_length = config.max_given_len + config.max_instr_len    
...
class NavCMT(BertPreTrainedModel):
    ...
    def forward(self, mode, txt_ids=None, txt_embeds=None, txt_masks=None,
                ...
                future_mask=None, graph_mask=None, history_mapper=None, question_ids=None, answer_ids=None):
        # text embedding            
        if mode == 'language':
        ...
        if mode == 't5-caption':
            ...
            # print("action_img_embeds", action_img_embeds.shape)
            # print("question_ids", question_ids.shape)
            # print("answer_ids", answer_ids.shape)

            # outputs = self.t5_with_image_model(
            #     input_ids=question_ids,
            #     attention_mask=None,  
            #     decoder_input_ids=answer_ids,
            #     labels=answer_ids,
            #     image_embedding=action_img_embeds
            # )
            # loss = outputs[0]
            # print('loss', loss)
            ...
            output = self.t5_with_image_model(
                input_ids=question_ids,
                attention_mask=None,  
                decoder_input_ids=answer_ids,
                labels=answer_ids,
        ...
        # cross-modal encoding per step
            ...
            if ob_masks is not None:
                extended_ob_masks = ob_masks.unsqueeze(1).unsqueeze(2)
                extended_ob_masks = extended_ob_masks.to(dtype=self.dtype, device=extended_action_masks.device)
            else:
                ob_embeds = torch.tensor([]).to(hist_masks.device)
                extended_ob_masks = torch.tensor([]).to(hist_masks.device)
            ...
            if graph_mask is not None:
                graph_max_size = graph_mask.size(-1)
                graph_mask = graph_mask.to(extended_action_img_masks.device)
                visn_self_attn_mask[:, 0, :graph_max_size, :graph_max_size] *= graph_mask
