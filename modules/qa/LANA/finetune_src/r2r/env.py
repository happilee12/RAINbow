...
from r2r.data_utils import load_nav_graphs, new_simulator, angle_feature, get_all_point_angle_feature
    ...
    def _get_obs(self, t=None, shortest_teacher=False):
        ...
        for i, (feature, state) in enumerate(self.env.getStates()):
            ...
            obs.append({
                ...
                'gt_path': item['path'],
                'cache_key': ','.join(item['path']),
                'path_id': item['path_id'],
                'position': abs_position,
                'goal_position': goal_pos,
                'path_to_goal': item['path_to_goal']
            })
            if 'instr_encoding' in item:
                obs[-1]['instr_encoding'] = item['instr_encoding']

            if 'given_encoding' in item:
                obs[-1]['given_encoding'] = item['given_encoding']
                obs[-1]['given'] = item['given']
            ...
            # print("obs[-1]['gt_path']", obs[-1]['gt_path'])
        return obs
...
class R2RBackBatch(R2RBatch):
    ...
    def eval_metrics(self, preds):
        ...
        return avg_metrics, metrics, _




class SpeakerBatch(object):
    ''' Implements the Room to Room navigation task, using discretized viewpoints and pretrained features '''

    def __init__(
            self, feat_db, connectivity_dir, scan_list, 
            batch_size=64, angle_feat_size=4,
            seed=0
    ):
        self.env = EnvBatch(connectivity_dir, feat_db=feat_db, batch_size=batch_size)
    ...
    def _shortest_path_action(self, scanId, viewpointId, goalViewpointId):
        ''' Determine next action on the shortest path to goal, for supervised training. '''
        if viewpointId == goalViewpointId:
            return goalViewpointId  # Just stop here
        path = self.shortest_paths[scanId][viewpointId][goalViewpointId]
        nextViewpointId = path[1]
        return nextViewpointId
    ...
    def reset(self, scanIds, viewpointIds, headings, goals=[]):
        self.batch_size = len(scanIds)
        self.env.newEpisodes(scanIds, viewpointIds, headings)
        return self._get_obs(t=0)
