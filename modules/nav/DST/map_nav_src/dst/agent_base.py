...
class BaseAgent(object):
    ...
    def get_results(self, detailed_output=False, wta_test=False):
        output = []
...
class Seq2SeqAgent(BaseAgent):
    ...
    def test(self, use_dropout=False, feedback='argmax', allow_cheat=False, iters=None, viz=False, **kwargs):
        ...
        if viz:
            ...
            super().test(iters=iters, **kwargs)


    def train(self, n_iters, feedback='teacher', train_alg='dagger', grad_accum_steps=1, **kwargs):
        ...
        # OOM counter for this training session
        oom_count = 0

        total_iters = n_iters * grad_accum_steps
        for iter in range(1, total_iters + 1):
            ...
            try:
                if train_alg == 'imitation':
                    self.feedback = 'teacher'
                    self.rollout(
                        train_ml=1., train_rl=False, **kwargs
                    )
                elif train_alg == 'imitation_shortest':
                    self.feedback = 'teacher_shortest'
                    self.rollout(
                        train_ml=1., train_rl=False, **kwargs
                    )
                elif train_alg == 'dagger': 
                ...
                self.loss.backward()
                accum_loss+= self.loss.item()
            except torch.cuda.OutOfMemoryError as e:
                oom_count += 1
                # print(f"OOM Error in rollout at iteration {iter}: {str(e)}")
                torch.cuda.empty_cache()
                continue
                
            # print(f"Accumulated Loss at iter {iter}: {accum_loss / grad_accum_steps}")
            if iter % grad_accum_steps == 0 or iter == total_iters: 
                ...
                #     print_progress(iter, total_iters+1, prefix='Progress:', suffix='Complete', bar_length=50)
        
        # Add OOM count to logs
        # if oom_count > 0:
            # if 'oom_count' not in self.logs:    
            #     self.logs['oom_count'] = []
        self.logs['oom_count'].append(oom_count)
            # print(f"Training completed with {oom_count} OOM errors")
    ...
    def load(self, path):
        ...
        states = torch.load(path, map_location=lambda storage, loc: storage, weights_only=True)
