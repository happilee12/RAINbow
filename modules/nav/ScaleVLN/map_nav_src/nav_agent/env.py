...
from sv_utils.data import load_nav_graphs, new_simulator
from sv_utils.data import angle_feature, get_all_point_angle_feature
import math
import random
import networkx as nx
from collections import defaultdict
import copy
import MatterSim
from nav_agent.eval_utils import cal_dtw, cal_cls
...
class R2RNavBatch(object):
    ...
    def _next_minibatch(self, train=False, batch_size=None, **kwargs):
        """
...
class NDHNavBatch(R2RNavBatch):
    def __init__(self, feat_db, instr_data, connectivity_dir,
        ...
        self.use_nav_turn_path = use_nav_turn_path
    ...
    def _next_minibatch(self, batch_size=None, **kwargs):
        ...
        if 'holistic' in kwargs and kwargs['holistic']:
            None
        else:
            for item in batch:
                if self.use_nav_turn_path: # use user path for training
                    item['path'] = item['_nav_turn']
                else:
                    item['path'] = item['gt_path']
                item['heading'] = 3.141592653589793 # item['start_heading']
        self.batch = batch
    ...
    def _get_obs(self, t=None, shortest_teacher=False):
        ...
        for i, (feature, state) in enumerate(self.env.getStates()):
            ...
            obs.append({
                ...
                'instruction' : item['instruction'],
                'instr_encoding': item['instr_encoding'],
                'target_only_instruction' : item['target_only_instruction'],
                'target_only_instr_encoding' : item['target_only_instr_encoding'],
                'path' : item['path'],
                'gt_path' : item['path'], # todo delete after val
                'path_id' : item['path_id'],
                # 'wta' : item['wta'] if 'wta' in item else False
                'wta' : item['_chat_idx'] != item['_chat_len'], # Not the last navigation
                'target' : item['target'],
            ...
            if 'end_panos' in item:
                ...
                for end_pano in item['end_panos']:
                    if end_pano in self.shortest_distances[scan][viewpoint]: 
    ...
    def _eval_item(self, scan, path, end_panos):
        ...
        for end_pano in end_panos:
            if shortest_distances[start_pano][end_pano] < gt_length:
                gt_path = shortest_paths[start_pano][end_pano]
                gt_length = shortest_distances[start_pano][end_pano]
        ...
        scores['spl'] = scores['success'] * gt_length / max(scores['trajectory_lengths'], gt_length, 0.01)
        scores['nav_error'] = np.min([shortest_distances[path[-1]][end_pano] for end_pano in end_panos])
        scores['gp'] = gt_length - np.min([shortest_distances[path[-1]][end_pano] for end_pano in end_panos])
        scores['gt_lengths'] = int(gt_length)
    ...
    def eval_metrics(self, preds, wta_mode=False):
        ''' Evaluate each agent trajectory based on how close it got to the goal location 
        the path contains [view_id, angle, vofv]'''
        print('eval %d predictions' % (len(preds)))
        ...
        for item in preds:
            ...
            try:
                ...
                # if wta_mode and item['stopped']:
                #     traj_scores['success'] = 0
                #     traj_scores['nav_error'] = np.nan
                ...
                print(e)
            if 'wta_label' in item:
                # print("wta_label", item['wta_label'], item['wta_predict'])
                metrics['wta_label'].extend(item['wta_label'])
                metrics['wta_predict'].extend(item['wta_predict'])
        
        
        wta_match = [1 if l == p else 0 for l, p in zip(metrics['wta_label'], metrics['wta_predict'])]
        avg_metrics = {
            'count': len(preds),
            ...
            'SDTW': np.mean(metrics['SDTW']) * 100,
            'CLS': np.mean(metrics['CLS']) * 100,
            'wta_accuracy': np.mean(wta_match) * 100,
        }
        return avg_metrics, metrics
