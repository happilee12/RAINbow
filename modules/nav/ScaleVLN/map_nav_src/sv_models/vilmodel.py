...
########
class QuestionDecisionHead(nn.Module):
    def __init__(self, hidden_size, input_size=None):
        super(QuestionDecisionHead, self).__init__()
        if input_size is None:
    ...
    def forward(self, global_feats, local_feats):
        # 글로벌과 로컬 특성을 결합
        combined = torch.cat([global_feats, local_feats], dim=-1) 
        return self.net(combined)

class GlocalTextPathNavCMTWTA(GlocalTextPathNavCMT):
    def __init__(self, config):
        super(GlocalTextPathNavCMTWTA, self).__init__(config)
        
        # 새로운 질문 결정 헤드 추가
        self.question_decision_head = QuestionDecisionHead(
            hidden_size=self.config.hidden_size,
            input_size=self.config.hidden_size * 2  # 글로벌과 로컬을 결합
        )
        
        # 새로운 헤드의 가중치 초기화
        self.init_weights()
        
        # 설정에 따라 새로운 헤드의 파라미터 고정
        if getattr(config, 'fix_question_decision_head', False):
            for name, param in self.question_decision_head.named_parameters():
                param.requires_grad = False

    @classmethod
    def from_pretrained(cls, pretrained_model_name_or_path, **kwargs):
        model = super(GlocalTextPathNavCMTWTA, cls).from_pretrained(pretrained_model_name_or_path, **kwargs)
        return model
    ...
    def forward_navigation_per_step(
        ...
        # 기존의 네비게이션 로직을 호출
        outs = super(GlocalTextPathNavCMTWTA, self).forward_navigation_per_step(
        ...
        # **질문 결정 헤드 적용**
        # 글로벌과 로컬의 첫 번째 임베딩(예: [CLS] 토큰 또는 대표 임베딩)을 사용
        global_feat = outs['gmap_embeds'][:, 0, :]  # (batch_size, hidden_size)
        local_feat = outs['vp_embeds'][:, 0, :]     # (batch_size, hidden_size)
        question_logits = self.question_decision_head(global_feat, local_feat)  # (batch_size, 2)
        
        # 소프트맥스를 적용하여 확률로 변환
        question_probs = torch.softmax(question_logits, dim=-1)  # (batch_size, 2)
        
        # 출력 딕셔너리에 추가
        outs['question_logits'] = question_logits
        outs['question_probs'] = question_probs
